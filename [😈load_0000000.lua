
gg.toast("🛡️SCRIPT CAR PARKING🛡️")
function START()
vip = gg.choice({
	"👨‍💻ฟังชั่น แฮ็กเงิน 💵 แฮ็กเหรียญ 💰",
	"🎃ฟังชั่น รถ 64BIT",
	"📖ฟังชั่น เกม",
	SPEEDA.."เร่งความเร็วรถสำหรับการแข่งขัน ⚡",
	 "━━━━━━━━━━━━━\n🌚 ออกจากสคริปต์\n์━━━━━━━━━━━━━"
},nil," ━━━━━━━━━━━━━━━━━━━━━\n สร้างสคริปต์โดย : Pbth Kick\n เกม : Car Parking \n เวอร์ชั่น 4.8.5.2\n━━━━━━━━━━━━━━━━━━━━━")

 if vip == nil then else end
 if vip == 1 then AZDXXXX() end
 if vip == 2 then GETXXX() end
 if vip == 3 then GETXXDSACX() end
 if vip == 4 then AZHD() end
 if vip == 5 then Exit() end
 GHOST = -1
 end



function AZDXXXX()
HACXQ = gg.choice({
	"😈‍ฟังชั่น แฮ็กเงิน 💵",
	"😈‍ฟังชั่น แฮ็กเหรียญ 💰",
	"↩️กลับ",
   "🗑️ออกจากสคริปต์"
},nil,"━━━━━━━━━━━━━━━━━━━━━\n สร้างสคริปต์โดย : Pbth Kick\n 👩‍💻ฟังชั่น แฮ็กเงิน แฮ็กเหรียญ\n━━━━━━━━━━━━━━━━━━━━━")

 if HACXQ == nil then else end
 if HACXQ == 1 then AZ() end
 if HACXQ == 2 then BB() end
 if HACXQ == 3 then START() end
 if HACXQ == 4 then Exit() end
GHOST = -1
 end





function AZ()
AA = gg.choice({
	"🩸10 ล้าน",
	"🩸20 ล้าน",
	"🩸30 ล้าน",
	"🩸40 ล้าน",
	"🩸50 ล้าน",
	"🛠️กำหนดเอง",
    "🌀ลบเงินทั้งหมด",
    "↩️กลับ",
    "🗑️ออกจากสคริปต์"
},nil,"👩‍💻ฟังชั่น แฮ็กเงิน")

 if AA == nil then else end
 if AA == 1 then Hs() end
 if AA == 2 then sd() end
 if AA == 3 then hg() end
 if AA == 4 then mm() end
 if AA == 5 then kk() end
 if AA == 6 then MAXXXX() end
 if AA == 7 then MAX() end
 if AA == 8 then START() end
 if AA == 9 then Exit() end
 GHOST = -1
 end



function Hs()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.setVisible(false)
gg.searchNumber('60', gg.TYPE_FLOAT)
gg.getResults(200)
gg.editAll('340000', gg.TYPE_FLOAT)
gg.clearResults()
gg.toast('แฮ็กเวลาเพิ่มเงินสำเร็จ')
end

function sd()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.setVisible(false)
gg.searchNumber('60', gg.TYPE_FLOAT)
gg.getResults(200)
gg.editAll('670000', gg.TYPE_FLOAT)
gg.clearResults()
gg.toast('แฮ็กเวลาเพิ่มเงินสำเร็จ')
end

function hg()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.setVisible(false)
gg.searchNumber('60', gg.TYPE_FLOAT)
gg.getResults(200)
gg.editAll('1000000', gg.TYPE_FLOAT)
gg.clearResults()
gg.toast('แฮ็กเวลาเพิ่มเงินสำเร็จ')
end

function mm()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.setVisible(false)
gg.searchNumber('60', gg.TYPE_FLOAT)
gg.getResults(200)
gg.editAll('1340000', gg.TYPE_FLOAT)
gg.clearResults()
gg.toast('แฮ็กเวลาเพิ่มเงินสำเร็จ')
end

function kk()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.setVisible(false)
gg.searchNumber('60', gg.TYPE_FLOAT)
gg.getResults(200)
gg.editAll('1670000', gg.TYPE_FLOAT)
gg.clearResults()
gg.toast('แฮ็กเวลาเพิ่มเงินสำเร็จ')
end

function MAXXXX()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.setVisible(false)
  gg.searchNumber("60", gg.TYPE_FLOAT)
  gg.getResults(200)
  input=gg.prompt({" "},
  {[1]=""})
  gg.editAll(input[1] ,gg.TYPE_FLOAT)
  gg.clearResults()
  gg.toast("กำหนดแฮ็กเวลาเพิ่มเงินสำเร็จ")
end

function MAX()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.setVisible(false)
gg.searchNumber('60', gg.TYPE_FLOAT)
gg.getResults(200)
gg.editAll('999999999', gg.TYPE_FLOAT)
gg.clearResults()
gg.toast('กำหนดแฮ็กเวลาลบสำเร็จ')
end

function BB()
  DF = gg.choice({
    "🌕 1 แสน",
    "🌕 2 แสน",
    "🌕 3 แสน",
    "🌕 4 แสน",
    "🌕 5 แสน",
    "🌀กำหนดเอง",
    "↩️กลับ",
	"🗑️ออกจากสคริปต์"
  }, nil, "👩‍💻ฟังชั่น แฮ็กเหรียญทอง")
  if DF == nil then else end
 if DF == 1 then FD() end
 if DF == 2 then FH() end
 if DF == 3 then SY() end
 if DF == 4 then BL() end
 if DF == 5 then SG() end
 if DF == 6 then Toolse() end
 if DF == 7 then START() end
 if DF == 8 then Exit() end
 GHOST = -1
 end



function FD()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.setVisible(false)
  gg.alert("ไปร้านซื้อรถ")
  gg.sleep(5000)
  gg.alert("ต่อไปเลือกรถตบแต่งที่ใช้ 30 เหรียญ")
  gg.sleep(5000)
  gg.searchNumber("30", gg.TYPE_DWORD)
  gg.getResults(100)
  gg.alert("กดเลือกของแต่งชิ้นที่มีราคา 50 เหรียญ ")
  gg.sleep(5000)
  gg.refineNumber("50", gg.TYPE_DWORD)
  gg.getResults(2)
  gg.editAll("-100000", gg.TYPE_DWORD)
  gg.clearResults(2)
  gg.alert("กดซื้อ แล้วเซฟข้อมูลเกม")
end

function FH()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.setVisible(false)
  gg.alert("ไปร้านซื้อรถ")
  gg.sleep(5000)
  gg.alert("ต่อไปเลือกรถตบแต่งที่ใช้ 30 เหรียญ")
  gg.sleep(5000)
  gg.searchNumber("30", gg.TYPE_DWORD)
  gg.getResults(100)
  gg.alert("กดเลือกของแต่งชิ้นที่มีราคา 50 เหรียญ ")
  gg.sleep(5000)
  gg.refineNumber("50", gg.TYPE_DWORD)
  gg.getResults(2)
  gg.editAll("-200000", gg.TYPE_DWORD)
  gg.clearResults(2)
  gg.alert("กดซื้อ แล้วเซฟข้อมูลเกม")
end

function SY()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.setVisible(false)
  gg.alert("ไปร้านซื้อรถ")
  gg.sleep(5000)
  gg.alert("ต่อไปเลือกรถตบแต่งที่ใช้ 30 เหรียญ")
  gg.sleep(5000)
  gg.searchNumber("30", gg.TYPE_DWORD)
  gg.getResults(100)
  gg.alert("กดเลือกของแต่งชิ้นที่มีราคา 50 เหรียญ ")
  gg.sleep(5000)
  gg.refineNumber("50", gg.TYPE_DWORD)
  gg.getResults(2)
  gg.editAll("-300000", gg.TYPE_DWORD)
  gg.clearResults(2)
  gg.alert("กดซื้อ แล้วเซฟข้อมูลเกม")
end

function BL()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.setVisible(false)
  gg.alert("ไปร้านซื้อรถ")
  gg.sleep(5000)
  gg.alert("ต่อไปเลือกรถตบแต่งที่ใช้ 30 เหรียญ")
  gg.sleep(5000)
  gg.searchNumber("30", gg.TYPE_DWORD)
  gg.getResults(100)
  gg.alert("กดเลือกของแต่งชิ้นที่มีราคา 50 เหรียญ ")
  gg.sleep(5000)
  gg.refineNumber("50", gg.TYPE_DWORD)
  gg.getResults(2)
  gg.editAll("-4000000", gg.TYPE_DWORD)
  gg.clearResults(2)
  gg.alert("กดซื้อ แล้วเซฟข้อมูลเกม")
end

function SG()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.setVisible(false)
  gg.alert("ไปร้านซื้อรถ")
  gg.sleep(5000)
  gg.alert("ต่อไปเลือกรถตบแต่งที่ใช้ 30 เหรียญ")
  gg.sleep(5000)
  gg.searchNumber("30", gg.TYPE_DWORD)
  gg.getResults(100)
  gg.alert("กดเลือกของแต่งชิ้นที่มีราคา 50 เหรียญ ")
  gg.sleep(5000)
  gg.refineNumber("50", gg.TYPE_DWORD)
  gg.getResults(2)
  gg.editAll("-500000", gg.TYPE_DWORD)
  gg.clearResults(2)
  gg.alert("กดซื้อ แล้วเซฟข้อมูลเกม")
end

function Toolse()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.setVisible(false)
  gg.alert("ไปร้านซื้อรถ")
  gg.sleep(5000)
  gg.alert("ต่อไปเลือกรถตบแต่งที่ใช้ 30 เหรียญ")
  gg.sleep(5000)
  gg.searchNumber("30", gg.TYPE_DWORD)
  gg.getResults(100)
  gg.alert("กดเลือกของแต่งชิ้นที่มีราคา 50 เหรียญ ")
  gg.sleep(5000)
  gg.refineNumber("50", gg.TYPE_DWORD)
  gg.getResults(2)
  input=gg.prompt({"ก่อน กำหนดเอง เติม - ก่อน"},
  {[1]=""})
  gg.editAll(input[1] ,gg.TYPE_DWORD)
  gg.clearResults(2)
  gg.alert("กดซื้อ แล้วเซฟข้อมูลเกม")
  gg.toast("กำหนด สำเร็จ")
end


SPEEDA = "🔴〘•กำลังปิดใช้งาน•〙"
 function AZHD()
if SPEEDA == "🔴〘•กำลังปิดใช้งาน•〙" then
      gg["setRanges"](gg["REGION_CODE_APP"])
      gg["searchNumber"]("0.3", gg["TYPE_FLOAT"], false, gg["SIGN_EQUAL"], 0, -1)
      gg["getResults"](100)
      gg["editAll"]("3.4100999", gg["TYPE_FLOAT"])
      gg["clearResults"]()
      gg["toast"]("✅เปิด")
 SPEEDA = "🔵〘•เปิดใช้งาน•〙"
 elseif SPEEDA == "🔵〘•เปิดใช้งาน•〙" then
      gg["setRanges"](gg["REGION_CODE_APP"])
      gg["searchNumber"]("3.4100999", gg["TYPE_FLOAT"], false, gg["SIGN_EQUAL"], 0, -1)
      gg["refineNumber"]("3.4100999", gg["TYPE_FLOAT"], false, gg["SIGN_EQUAL"], 0, -1)
      gg["getResults"](100)
      gg["editAll"]("0.3", gg["TYPE_FLOAT"])
      gg["clearResults"]()
      gg["toast"]("❌ปิด")
 SPEEDA = "🔴〘•กำลังปิดใช้งาน•〙"
 end
end



function GETXXDSACX()
DGKOPSHOP = gg.choice({
    "🍁ฟังชั่น ดัดแปลงชื่อยาว",
    "↩️กลับ",
    "🗑️ออกจากสคริปต์"
},nil,"━━━━━━━━━━━━━━━━━━━━━\n สร้างสคริปต์โดย : Pbth Kick\n━━━━━━━━━━━━━━━━━━━━━")

 if DGKOPSHOP == nil then else end
 if DGKOPSHOP == 1 then GETXXDSACXDGKOPSHOP() end
 if DGKOPSHOP == 2 then START() end
 if DGKOPSHOP == 3 then Exit() end
GHOST = -1
 end

function GETXXDSACXDGKOPSHOP()
  gg.setVisible(false)
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.toast("พิมพ์ 1 ")
  gg.sleep(5000)
  gg.searchNumber("12;1", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
  gg.toast("พิมพ์ 123")
  gg.sleep(5000)
  gg.refineNumber("12;3", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
  gg.refineNumber("12", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
  gg.getResults(100)
  input=gg.prompt({"ใส่ตัวเลขเท่าไหร่ก็ได้ที่คุณอยากกำหนด"},
  {[1]=""})
  gg.editAll(input[1] ,gg.TYPE_DWORD)
  gg.clearResults()
  gg.alert("ตอนนี้ลบชื่อเก่าของ|คุณสามารถตั้งชื่อของคุณได้นานเท่าที่คุณต้องการ")
end

function GETXXX()
HACXDGETXXXQ = gg.choice({
	"🔥ฟังชั่น ดัดแปลงรถ เพิ่มความเร็วรถ",
	"🔥ฟังชั่น ปลดล็อครถเติม [ รออัพเดท ]",
	"🔥ฟังชั่น ล้อรถ UFO",
    "🔥์ฟังชั่น ถอดหลังคารถ ลอย",
    "🔥ฟังชั่น สีโครมล้อรถ",
    "🔥ฟังชั่น สีเรืองแสงรถ",
   	"↩️กลับ",
    "🗑️ออกจากสคริปต์"
},nil,"━━━━━━━━━━━━━━━━━━━━━\n สร้างสคริปต์โดย : Pbth Kick\n━━━━━━━━━━━━━━━━━━━━━")

 if HACXDGETXXXQ == nil then else end
 if HACXDGETXXXQ == 1 then CARVIPC() end
 if HACXDGETXXXQ == 2 then DFHJHGKKKITSS() end
 if HACXDGETXXXQ == 3 then SADRRR() end
 if HACXDGETXXXQ == 4 then yhkodjkufjjugg() end
 if HACXDGETXXXQ == 5 then SOODKFKDKWO() end
 if HACXDGETXXXQ == 6 then SXXDKFKDKWO() end
 if HACXDGETXXXQ == 7 then START() end
 if HACXDGETXXXQ == 8 then Exit() end
GHOST = -1
 end

function SXXDKFKDKWO()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.searchNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
input=gg.prompt({"โค้ดสีฟ้า: 200;200;-200\nโค้ดสีแดงออกดำ: 100;-100;-100\nโค้ดสีเขียวออกเหลือง: 100;100;-100\nโค้ดสีฟ้าออกขาว: 100;100;100\nโค้ดสีน้ำเงิน: -200;-200;200\nโค้ดสีฟ้าเขียว: -100;100;-100"},
{[1]=""})
gg.editAll(input[1] ,gg.TYPE_FLOAT)
end




function SOODKFKDKWO()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.searchNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีดำ")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("0",gg.TYPE_FLOAT)
gg.sleep(50)
gg.toast("สีขาว")
gg.sleep(2250)
gg.toast("กำลังจะเริ่ม ค้นหาใน 3 วิ")
gg.sleep(250)
gg.toast("1")
gg.sleep(250)
gg.toast("1..2")
gg.sleep(250)
gg.toast("1..2...3")
gg.sleep(250)
gg.toast("ค้นหา........")
gg.refineNumber("1",gg.TYPE_FLOAT)
input=gg.prompt({"โค้ดสีฟ้า: 200;200;-200\nโค้ดสีเหลือง: 100;-100;100\nโค้ดสีแดง: -200;-200;200\nโค้ดสีม่วง: -200;200;200"},
{[1]=""})
gg.editAll(input[1] ,gg.TYPE_FLOAT)
end


function SADRRR()
  gg.setVisible(false)
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.alert("ไปตั้งค่าทุกอย่างให้สูงสุดยกเว้น TRAVE ปรับ ค่าเริ่มต้นแล้วรอสักครู่")
  gg.searchNumber('-10', gg.TYPE_FLOAT)
  gg.getResults(100)
  gg.editAll('-90', gg.TYPE_FLOAT)
  gg.refineNumber('-10', gg.TYPE_FLOAT)
  gg.getResults(100)
  gg.editAll('-90', gg.TYPE_FLOAT)
  gg.clearResults()
  gg.toast('ทำ ล้อ UFO สำเร็จ')
end

function yhkodjkufjjugg()
gg.setVisible(false)
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.alert("ตอนนี้เลือกโรงรถแล้วไปที่ชุดแต่งรอบคัน")
  gg.sleep(2000)
  gg.alert("2 บัฟเฟอร์หมายเลข กดย้ำๆ")
  gg.sleep(4500)
  gg.searchNumber("1",gg.TYPE_DWORD)
  gg.alert("3 บัฟเฟอร์หมายเลข กดย้ำๆ")
  gg.sleep(4500)
  gg.refineNumber("2",gg.TYPE_DWORD)
  gg.alert("4 บัฟเฟอร์หมายเลข กดย้ำๆ")
  gg.sleep(4500)
  gg.refineNumber("3",gg.TYPE_DWORD)
  gg.alert("ไป 2 บัฟเฟอร์หมายเลข")
  gg.sleep(4500)
  gg.getResults(10)
  gg.editAll('1',gg.TYPE_DWORD)
  gg.alert("กดซื้อ 2 บัฟเฟอร์หมายเลข")
  gg.sleep(50)
  gg.alert("ออกจากหน้าต่างนี้ ไปเปลี่ยนรถ แล้วคันเก่าคืน")
  gg.sleep(250)
  gg.alert("ไปที่ แตกต่างรถ เลือก KIT ชิ้น บน สามารถเลือกสีได้ ")
  gg.sleep(25000)
  gg.editAll("6",gg.TYPE_DWORD)
  gg.clearResults()
  gg.alert("ตอนนี้ซื้อชั้นสูงสุดที่คุณเลือกแล้วออกจากที่นี่ไปเลือกรถคันอื่นและรับรถเก่าของคุณคืน")
  gg.toast("ปรับตั้งค่า สำเร็จ")
  end



function CARVIPC()
  CARVIP = gg.choice({
    "💀รถคันที่ #0 Free",
    "💀รถคันที่ #18 FIAT 131",
    "💀รถคันที่ #49 350z",
    "💀รถคันที่ #76 BMW W8",
    "💀รถคันที่ #104 Euroline Fullbox",
    "🔙 กลับ ",
    "🗑️ออกจากสคริปต์"
  }, nil, "กำลังอยู่ช่วงหาค่า รถต่างๆ ")
  if CARVIP == nil then else end
  if CARVIP == 1 then FrEe()  end
  if CARVIP == 2 then FIAT()  end
  if CARVIP == 3 then BMWWEIGHT()  end
  if CARVIP == 4 then BMWWEIGHTFIAT()  end
  if CARVIP == 5 then EurolineFullbox()  end
  if CARVIP == 6 then START() end
  if CARVIP == 7 then Exit() end
  GHOST = -1
  end



function FrEe()
gg.setVisible(false)
gg.toast("▂                                         10% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.toast("▂▃                                   20% ")
gg.searchNumber("175",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("1695",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅                             30% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("260",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("2899",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆                      40% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("5500",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("1001",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆██                   60% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("1400",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("2501",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP,gg.REGION_CODE_SYSTEM)
gg.searchNumber("0.1",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("9.999999E-29",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("5.00",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("3.10",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆█████     85% ")
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("2.66",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.03",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.91",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.00",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.39",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.toast("▂▃▅▆███████100%")
gg.getResults(100)
gg.editAll("6",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("กดรี 2 ครั้ง")
end

  function FIAT()
gg.setVisible(false)
gg.toast("▂                                         10% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.toast("▂▃                                   20% ")
gg.searchNumber("300",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("414",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅                             30% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("410",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("2254",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆                      40% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("7000",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("1000",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆██                   60% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("3000",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("2120",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP,gg.REGION_CODE_SYSTEM)
gg.searchNumber("0.1",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("9.999999E-29",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("5.00",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("3.10",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆█████     85% ")
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("2.66",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.03",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.91",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.00",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.39",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.toast("▂▃▅▆███████100%")
gg.getResults(100)
gg.editAll("6",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("กดรี 2 ครั้ง")
end

 function BMWWEIGHTFIAT()
gg.setVisible(false)
gg.toast("▂                                         10% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.toast("▂▃                                   20% ")
gg.searchNumber("305",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("925",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅                             30% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("360",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("1049",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆                      40% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("6800",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("5500",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆██                   60% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("3000",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("5600",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP,gg.REGION_CODE_SYSTEM)
gg.searchNumber("0.1",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("9.999999E-29",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("5.00",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("3.10",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆█████     85% ")
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("2.66",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.03",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.91",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.00",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.39",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.toast("▂▃▅▆███████100%")
gg.getResults(100)
gg.editAll("6",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("กดรี 2 ครั้ง")
end

function BMWWEIGHT()
gg.setVisible(false)
gg.toast("▂                                         10% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.toast("▂▃                                   20% ")
gg.searchNumber("620",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("414",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅                             30% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("800",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("2254",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆                      40% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("7000",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("1000",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆██                   60% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("2500",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("2120",gg.TYPE_FLOAT)
gg.clearResults(1000)
gg.setRanges(gg.REGION_CODE_APP,gg.REGION_CODE_SYSTEM)
gg.searchNumber("0.1",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("9.999999E-29",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("5.00",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("3.10",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆█████     85% ")
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("2.66",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.03",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.91",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.00",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.39",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.toast("▂▃▅▆███████100%")
gg.getResults(100)
gg.editAll("6",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("กดรี 2 ครั้ง")
end

 function EurolineFullbox()
gg.setVisible(false)
gg.toast("▂                                         10% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.toast("▂▃                                   20% ")
gg.searchNumber("280",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("925",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅                             30% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("340",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("1049",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆                      40% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("6700",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("5500",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆██                   60% ")
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("2000",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("5600",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP,gg.REGION_CODE_SYSTEM)
gg.searchNumber("0.1",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("9.999999E-29",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("5.00",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("3.10",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("▂▃▅▆█████     85% ")
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("2.66",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.03",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.91",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0.00",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.39",gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.toast("▂▃▅▆███████100%")
gg.getResults(100)
gg.editAll("6",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("กดรี 2 ครั้ง")
end


function DFHJHGKKKITSS()
DFHJHGKKKIT=gg.choice({
" 1.NISSAN 350z ",
" 2.DODGE VIPER ",
" 3.BMW M8 ",
" 4.GT 63 ",
" 5.BMW i8 ",
" 6.LAMBORGHINI URUS ",
" 7.TOYOTA CAMRY ",
" 8.BMW M5 ",
" 9.MERSCEDES BENZ #88 ",
" 10.BMW X6 ",
" 11.HUMMER H1 ",
" 12.NISSAN 200sx ",
" 13.INVISIBLE CAR ",
" 14.MERSCEDES BENZ 98 ",
"🔙 กลับ ",
"🗑️ออกจากสคริปต์"
 }, nil, "กำลังอยู่ช่วงทดลองใช้งาน👻")
  if DFHJHGKKKIT == nil then else end
if DFHJHGKKKIT ==  1 then ZA()  end
if DFHJHGKKKIT ==  2 then ZB()  end
if DFHJHGKKKIT ==  3 then ZC()  end
if DFHJHGKKKIT ==  4 then ZD()  end
if DFHJHGKKKIT ==  5 then ZE()  end
if DFHJHGKKKIT ==  6 then ZF()  end
if DFHJHGKKKIT ==  7 then ZG()  end
if DFHJHGKKKIT ==  8 then ZN()  end
if DFHJHGKKKIT ==  9 then ZI()   end
if DFHJHGKKKIT == 10 then ZL()  end
if DFHJHGKKKIT == 11 then ZM()    end
if DFHJHGKKKIT == 12 then ZNN()   end
if DFHJHGKKKIT == 13 then ZO()     end
if DFHJHGKKKIT == 14 then ZP()     end
if DFHJHGKKKIT == 15 then START() end
if DFHJHGKKKIT == 16 then Exit()    end
 GHOST = -1
 end



function ZA()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('325',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZB()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('321',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZC()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('324',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZD()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('322',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZE()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('312',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZF()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('293',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZG()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('288',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZN()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('260',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZI()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('276',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZL()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('280',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZM()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('281',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZNN()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('305',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZO()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('290',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end
function ZP()
gg.setVisible(false)
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber('134',gg.TYPE_DWORD)
gg.getResults(39)
gg.editAll('310',gg.TYPE_DWORD)
gg.clearResults(39)
gg.toast('กดซื้อ')
end





function Exit()
  gg.toast("ออกจากสคริปต์")
  print("  ┌─┐  .─┐")
  print("  │▒│ /▒/      ")
  print("  │▒│/▒/       ")
  print("  │▒/▒/─┬─┐")
  print("  │▒│▒|▒│▒│ ")
  print("┌┴─┴─┐-┘─┘  ")
  print("│▒┌──┘▒▒▒│ ")
  print("└┐▒▒▒▒▒▒┌┘")
  print("  └┐▒▒▒▒┌┘")
  os.exit()
end


while true do
  if gg.isVisible(true) then
    GHOST = 1
    gg.setVisible(false)
  end
  if GHOST == 1 then
    START()
  end
  GHOST = -1
end
